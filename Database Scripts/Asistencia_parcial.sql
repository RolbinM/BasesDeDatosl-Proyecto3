...
GO
/****** Object:  StoredProcedure [dbo].[sp_InsertarMarca]    Script Date: 6/16/2021 8:46:22 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[sp_InsertarMarca]
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN TRANSACTION InsertarMarca
			...
			SELECT @IdMesActual = (SELECT IdMesPlanilla FROM dbo.PlanillaXMesXEmpleado WHERE Id = @idMPXE)
			...
			BEGIN
				...
				*/
				...
				IF @diaMov = (SELECT FechaFin FROM MesPlanilla WHERE Id = @IdMesActual)  ---Arreglar Don Rolbin
				BEGIN
					SET @idMPXE = @idMPXE + 1
